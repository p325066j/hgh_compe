...
interface ConsultationWaitingTimeEditCardProps {
  waitingTime: ConsultationWaitingTimeWithRoom;
  onUpdate: (id: string, updates: Partial<ConsultationWaitingTimeWithRoom>) => void;
}

const ConsultationWaitingTimeEditCard: React.FC<ConsultationWaitingTimeEditCardProps> = ({ waitingTime, onUpdate }) => {
  const { id, room, waitingCount, estimatedMinutes, status } = waitingTime;
  ...
  const getDepartmentColor = (department: '内科' | '外科') => {
    return department === '内科' ? 'border-blue-500' : 'border-green-500';
  };

  return (
    <div className={`bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 mb-4 border-l-4 ${getDepartmentColor(room.department)} transition-colors`}>
      <div className="flex justify-between items-start mb-2">
        <div>
          <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200">
            診察室{room.roomNumber}
          </h3>
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">{room.department} - {room.doctorName} 先生</p>
        </div>
...
export default ConsultationWaitingTimeEditCard; 